...
void LLTexLayerParamAlpha::setWeight(F32 weight, BOOL upload_bake)
{
	...
	if (cur_u8 != new_u8)
	{
		...
		if ((mAvatar->getSex() & getSex()) && (mAvatar->isSelf() && !mAvatar->mIsDummy)) // only trigger a baked texture update if we're changing a wearable's visual param.
		{
...
BOOL LLTexLayerParamAlpha::getSkip() const
{
	...
	{
//impfixme 		F32 effective_weight = (avatar->getSex() & mAvatar->getSex()) ? mCurWeight : getDefaultWeight();
...
LLColor4 LLTexLayerParamColor::getNetColor() const
{
	...
	llassert(info->mNumColors >= 1);
 	...
 	F32 effective_weight; //impfixme = (mAvatar && (mAvatar->getSex() & getSex())) ? mCurWeight : getDefaultWeight();
