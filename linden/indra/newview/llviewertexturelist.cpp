...
void LLViewerTextureList::doPreloadImages()
{
	...
	image = LLViewerTextureManager::getFetchedTextureFromFile("noentrylines.j2c", MIPMAP_YES, LLViewerFetchedTexture::BOOST_UI);//SG2: "world/NoEntryLines.png"
	if (image) 
	{
		image->setAddressMode(LLTexUnit::TAM_WRAP);	
		mImagePreloads.insert(image);
	}
	image = LLViewerTextureManager::getFetchedTextureFromFile("noentrypasslines.j2c", MIPMAP_YES, LLViewerFetchedTexture::BOOST_UI);//SG2: "world/NoEntryPassLines.png"
	if (image) 
	{
		image->setAddressMode(LLTexUnit::TAM_WRAP);
		mImagePreloads.insert(image);
...
LLUIImagePtr LLUIImageList::getUIImageByID(const LLUUID& image_id)
{
	...
	return loadUIImageByID(image_id, use_mips, scale_rect);
}
...
LLUIImagePtr LLUIImageList::getUIImage(const std::string& image_name)
{
	...
	const BOOL use_mips = FALSE;
	const LLRect scale_rect = LLRect::null;
	return loadUIImageByName(image_name, image_name, use_mips, scale_rect);
}
