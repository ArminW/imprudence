...
void LLViewerFetchedTexture::processTextureStats()
{
	...
	static bool textures_fullres = gSavedSettings.getBOOL("TextureLoadFullRes");
...
{
	static bool textures_decode_disabled = gSavedSettings.getBOOL("TextureDecodeDisabled");
	...
	if (mIsFetching)
	{
		...
		{
			mFetchState = LLAppViewer::getTextureFetch()->getFetchState(mID, mDownloadProgress, mRequestedDownloadPriority,	mFetchPriority, mFetchDeltaTime, mRequestDeltaTime);
		}
...
// 		}
		...
		fetch_request_created  = LLAppViewer::getTextureFetch()->createRequest(mUrl, getID(),getTargetHost(), decode_priority,  w, h, c, desired_discard, needsAux());
