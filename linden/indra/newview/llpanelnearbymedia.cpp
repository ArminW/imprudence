...
// 	gSavedSettings.getControl(LLViewerMedia::AUTO_PLAY_MEDIA_SETTING)->getSignal()->connect(boost::bind(&LLPanelNearByMedia::handleMediaAutoPlayChanged, this, _2));
	...
	childSetAction("all_nearby_media_enable_btn", onClickEnableAll, this);
/*
...
void LLPanelNearByMedia::refreshParcelItems()
{
	...
	if (gSavedSettings.getBOOL("AudioStreamingVideo") &&should_include && LLViewerMedia::hasParcelMedia())
	{
...
void LLPanelNearByMedia::onClickEnableAll(void* userdata)
{
	LLViewerMedia::setAllMediaEnabled(true);
}

void LLPanelNearByMedia::onClickDisableAll(void* userdata)
{
	LLViewerMedia::setAllMediaEnabled(false);
}

void LLPanelNearByMedia::onClickEnableParcelMedia(void* userdata)
{	
...
void LLPanelNearByMedia::onClickDisableParcelMedia(void* userdata)
{	
...
{
// 	LLPanelNearByMedia* self = LLPanelNearByMedia::getInstance();
// 	if (row_id == PARCEL_AUDIO_LIST_ITEM_UUID)
// 	{
// 		if (disabled) self->onClickParcelAudioStop();
// 		else self->onClickParcelAudioStart();
// 		return true;
// 	}
// 	else if (row_id == PARCEL_MEDIA_LIST_ITEM_UUID)
// 	{
// 		if (disabled) self->onClickDisableParcelMedia();
// 		else self->onClickEnableParcelMedia();
// 		return true;
// 	}
// 	else {
// 		LLViewerMediaImpl* impl = LLViewerMedia::getMediaImplFromTextureID(row_id);
...
void LLPanelNearByMedia::onClickParcelMediaPlay(void* userdata)
{
	LLViewerParcelMedia::play(LLViewerParcelMgr::getInstance()->getAgentParcel());
}

void LLPanelNearByMedia::onClickParcelMediaStop(void* userdata)
{	
...
void LLPanelNearByMedia::onClickParcelMediaPause(void* userdata)
{
	LLViewerParcelMedia::pause();
}

void LLPanelNearByMedia::onClickParcelAudioStart(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	// User *explicitly* started the internet stream, so keep the stream
	// playing and updated as they cross to other parcels etc.
	self->mParcelAudioAutoStart = true;
...
void LLPanelNearByMedia::onClickParcelAudioPlay(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	// User *explicitly* started the internet stream, so keep the stream
	// playing and updated as they cross to other parcels etc.
...
void LLPanelNearByMedia::onClickParcelAudioStop(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	...
	self->mParcelAudioAutoStart = false;
...
void LLPanelNearByMedia::onClickParcelAudioPause(void* userdata)
{
	if (!gAudiop)
		return;
...
void LLPanelNearByMedia::onAdvancedButtonClick(void* userdata)
{	
...
void LLPanelNearByMedia::onMoreLess(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	bool is_more = self->getChild<LLUICtrl>("more_btn")->getVisible();
	self->mNearbyMediaPanel->setVisible(is_more);

	// enable resizing when expanded
	self->getChildView("resizebar_bottom")->setEnabled(is_more);

	LLRect new_rect = is_more ? self->mMoreRect : self->mLessRect;
	new_rect.translate(self->getRect().mRight - new_rect.mRight, self->getRect().mTop - new_rect.mTop);

// 	self->setShape(new_rect);

	self->getChild<LLUICtrl>("more_btn")->setVisible(!is_more);
	self->getChild<LLUICtrl>("less_btn")->setVisible(is_more);
}
...
void LLPanelNearByMedia::updateControls()
{
	...
	{
		if (!LLViewerMedia::hasParcelMedia() || !gSavedSettings.getBOOL("AudioStreamingVideo"))
		{
		...
		if (NULL == impl || !gSavedSettings.getBOOL("AudioStreamingVideo"))
		{
			showDisabledControls();
		}
		else {
...
void LLPanelNearByMedia::onClickSelectedMediaStop(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	self->setDisabled(self->mMediaList->getValue().asUUID(), true);
}

void LLPanelNearByMedia::onClickSelectedMediaPlay(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	...
	// First enable it
	self->setDisabled(selected_media_id, false);
...
void LLPanelNearByMedia::onClickSelectedMediaPause(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	...
	if (selected_media_id == PARCEL_AUDIO_LIST_ITEM_UUID)
	{
		self->onClickParcelAudioPause(userdata);
	}
	else if (selected_media_id == PARCEL_MEDIA_LIST_ITEM_UUID) 
	{
		self->onClickParcelMediaPause(userdata);
	}
	else {
		LLViewerMediaImpl* impl = LLViewerMedia::getMediaImplFromTextureID(selected_media_id);
...
void LLPanelNearByMedia::onClickSelectedMediaMute(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	...
	if (selected_media_id == PARCEL_AUDIO_LIST_ITEM_UUID)
	{
		gSavedSettings.setBOOL("MuteMusic", self->mMuteBtn->getValue());
	}
	else {
		LLViewerMediaImpl* impl = (selected_media_id == PARCEL_MEDIA_LIST_ITEM_UUID) ?
			((LLViewerMediaImpl*)LLViewerParcelMedia::getParcelMedia()) : LLViewerMedia::getMediaImplFromTextureID(selected_media_id);
		if (NULL != impl)
		{
			...
			else if (self->mVolumeSlider->getValueF32() == 0.0)
			{
				impl->setVolume(1.0);
				self->mVolumeSlider->setValue(1.0);
			}
			else 
			{
...
void LLPanelNearByMedia::onCommitSelectedMediaVolume(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
	...
	if (selected_media_id == PARCEL_AUDIO_LIST_ITEM_UUID)
	{
		F32 vol = self->mVolumeSlider->getValueF32();
...
void LLPanelNearByMedia::onClickSelectedMediaZoom(void* userdata)
{
	LLPanelNearByMedia* self = (LLPanelNearByMedia*)userdata;
...
void LLPanelNearByMedia::onClickSelectedMediaUnzoom(void* userdata)
{
