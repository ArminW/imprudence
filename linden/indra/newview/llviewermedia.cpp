...
#include "llparcel.h"
#include "llaudioengine.h"  // for gAudiop - orly?
...
//imp fixme std::string LLViewerMedia::sOpenIDCookie;
...
{	
	...
	{	
		...
		{
			LL_DEBUGS("Media") << "The current media URL is not empty" << LL_ENDL;
	...
	{
		LL_DEBUGS("Media") << "new MediaImpl" << LL_ENDL;
	...
	if(media_impl)
	{
		if(needs_navigate)
		{
			...
			LL_DEBUGS("Media") << "navigating to URL " << media_impl->mMediaEntryURL << LL_ENDL;
		}
		else if(!media_impl->mMediaURL.empty() && (media_impl->mMediaURL != media_impl->mMediaEntryURL))
		{
			...
			LL_DEBUGS("Media") << "updating URL in the media impl to " << media_impl->mMediaEntryURL << LL_ENDL;
		}
	}
	else LL_DEBUGS("Media") << "NO MediaImpl" << LL_ENDL;
...
bool LLViewerMedia::isInterestingEnough(const LLVOVolume *object, const F64 &object_interest)
{
	...
	{
		LL_DEBUGS("Media") << "object interest = " << object_interest << ", lowest loadable = " << sLowestLoadableImplInterest << LL_ENDL;
...
{
	LLFastTimer t1(LLFastTimer::FTM_MEDIA_UPDATE);
	...
	{
		LL_DEBUGS("Media") << "updated cookies will be sent to all loaded plugins: " << llendl;
		LL_DEBUGS("Media") << sUpdatedCookies << llendl;
	}
...
//SG2:	bool inworld_media_enabled = gSavedSettings.getBOOL("AudioStreamingMedia");
	...
	for(; iter != end; iter++)
	{
		...
		if(!inworld_media_enabled)
		{
			...
			{
				new_priority = LLPluginClassMedia::PRIORITY_UNLOADED;
				LL_DEBUGS("Media")<< "!inworld_media_enabled" << LL_ENDL;
			}
		}
...
{ 
	if (mMediaSource)
	{
		static const std::string name = mMediaSource->getMediaName();
		return name;
	}
...
void LLViewerMediaImpl::calculateInterest()
{
	...
	if(!mObjectList.empty())
	{
		...
		LLVector3d camera_delta = gAgent.getCameraPositionGlobal() - obj_global;
